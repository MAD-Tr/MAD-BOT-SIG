...
def pass_check(m):
    if m.text.split()[1] == PASSWORD if len(m.text.split())>1 else False:
        allowed.add(m.from_user.id)
        bot.reply_to(m, "✅ فتحت البوت /start")
    else:
        bot.reply_to(m, "❌ غلط")

MARKETS = {"EUR/USD":"EURUSD","GBP/USD":"GBPUSD","USD/JPY":"USDJPY","AUD/USD":"AUDUSD","USD/CAD":"USDCAD"}
user_data, last_request, quotex_settings = {}, {}, {}
...
def runner(uid, cid):
    try:
        ...
        bot.send_message(cid, f"✅ دخل كوتكس | {s['amount']} ر.س | {s['trades']} صفقات")
        done=0
...
def qstop(call): quotex_settings[call.from_user.id]["running"]=False
...
def cm(call):
    ...
    bot.send_message(call.message.chat.id, name, reply_markup=mk)
...
def ct(call):
    ...
    else: bot.edit_message_text(f"{name} {d} {p}%\n{det}", call.message.chat.id, loading.message_id)
