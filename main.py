...
def pass_check(m):
    try:
        if m.text.split()[1] == PASSWORD:
            ...
            bot.reply_to(m, "✅ تم، الحين اكتب /start")
        else:
            bot.reply_to(m, "❌ الرقم غلط")
    except:
...
def q_menu(uid):
    ...
    mk.add(
        InlineKeyboardButton("10", callback_data="qa_10"),
        InlineKeyboardButton("25", callback_data="qa_25"),
        InlineKeyboardButton("50", callback_data="qa_50")
    )
    mk.add(InlineKeyboardButton(f"🔢 عدد الصفقات {s['trades']}", callback_data="none"))
    mk.add(
        InlineKeyboardButton("2", callback_data="qt_2"),
        InlineKeyboardButton("4", callback_data="qt_4"),
        InlineKeyboardButton("6", callback_data="qt_6")
    )
...
def qa_menu(call):
    ...
    if call.from_user.id not in qs:
        qs[call.from_user.id] = {"amount": 25, "trades": 4, "running": False, "email": None}
    try:
...
def login(m):
    p = m.text.split()
    if len(p) < 3:
        return bot.reply_to(m, "❌ الصيغة: /login الايميل الباسورد")
    if m.from_user.id not in qs:
        qs[m.from_user.id] = {"amount": 25, "trades": 4, "running": False}
    qs[m.from_user.id]["email"] = p[1]
    qs[m.from_user.id]["password"] = p[2]
    bot.reply_to(m, f"✅ تم حفظ {p[1]}")
...
def runner(uid, cid):
    try:
        ...
        ok, rea = qx.connect()
        if not ok:
            return bot.send_message(cid, f"❌ فشل الدخول: {rea}")
        bot.send_message(cid, f"✅ تم الدخول | رصيد: {qx.get_balance()} | {s['amount']} ر.س")
        done = 0
        while qs[uid]["running"] and done < s["trades"]:
            for name, sym in MARKETS.items():
                if not qs[uid]["running"] or done >= s["trades"]:
                    break
                ...
                if d!= "NO_TRADE" and per >= 85:
                    ...
                    if st:
                        ...
                        bot.send_message(cid, f"✅ صفقة #{done} {name} {d} {per}%")
                        time.sleep(70)
            time.sleep(10)
        qs[uid]["running"] = False
        bot.send_message(cid, "🏁 خلصت كل الصفقات")
...
def qs_start(call):
    if qs[call.from_user.id].get("email") is None:
        return bot.send_message(call.message.chat.id, "لازم /login اول")
    ...
    while True:
        try:
            ...
            if "409" in msg or "Conflict" in msg:
                logging.warning("409 Conflict - waiting 20s")
                time.sleep(20)
            else:
