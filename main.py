...
def get_tf_gainz_v3(symbol, interval):
    # اصلاح GOLD OTC - يجرب XAUUSD
    if symbol == "GOLD":
        try_symbols = ["XAUUSD", "GOLD"]
        exchanges = ["OANDA", "FX", "TVC", "FX_IDC"]
        screener = "forex"
    else:
        try_symbols = [symbol]
    ...
    for sym_try in try_symbols:
        for exchange in exchanges:
            try:
                h = TA_Handler(symbol=sym_try, screener=screener, exchange=exchange, interval=interval)
                a = h.get_analysis()
                ind = a.indicators
...
def get_signal(symbol):
    ...
    if "ERROR" in [d5,d15,d1h]:
        return "NO_TRADE",0,"⚠️ TradingView معلق جرب بعد شوي"
...
def main_menu(chat_id):
    ...
    bot.send_message(chat_id,"🏆 MAD-BOT V3 - 14 + GOLD OTC",reply_markup=m)
...
def calls(call):
    ...
    if call.data=="golden":
        ...
        m.add(InlineKeyboardButton("🔥 عادي",callback_data="golden"))
        bot.edit_message_text(txt, call.message.chat.id, load.message_id, reply_markup=m)
...
app=Flask(__name__)
@app.route('/')
def h(): return "Live V3 15 Markets Fixed"
